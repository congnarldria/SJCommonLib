...
namespace SJCommonLib
{
    ...
    public static class ATExtensions
    {
        ...
        public static T DeepClone<T>(this T item)
        {
            ...
            {
                LogMgr.SendLog("Extension", "Clone Fail = " + e.Message, e);
            }
            return default;
        }
        /// <summary>
        /// Xml 深層複製的泛型方法 ，用於所有成員都不都是可序列化時，所有參考會消失，要重新參考
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="item"></param>
        /// <returns></returns>
        public static T DeepCloneXml<T>(this T item)
        {
            ...
            {
                LogMgr.SendLog("Extension", "XmlClone Fail = " + e.Message, e);
            }
            return default;
        }
        public static void XmlSeriallize<T>(this T item, string FileName)
        {
            try
            {
            ...
            {
                LogMgr.SendLog("Extension", e.Message, e);
            }
        }
        public static T XmlDeSeriallize<T>(string FileName)
        {
            try
            {
                if (Directory.Exists(Path.GetDirectoryName(FileName)))
                {
                    ...
                    using (FileStream myFileStream = new FileStream(FileName, FileMode.Open))
                    {
                        T item = (T)ser.Deserialize(myFileStream);
                        return item;
                    }
                }
            }
        ...
        public static void JsonSeriallize<T>(this T item, string FileName)
        {
            try
            {
                if (Directory.Exists(Path.GetDirectoryName(FileName)))
        ...
        public static T JsonDeSeriallize<T>(string FileName)
        {
            try
            {
                if (Directory.Exists(Path.GetDirectoryName(FileName)))
                {
                    using (FileStream myFileStream = new FileStream(FileName, FileMode.Open))
                    {
                        T item =  JsonConvert.DeserializeObject<T>(FileName);
                        return item;
                    }
                }
            }
